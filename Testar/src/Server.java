...
//O PROJETO É EXECUTADO PELA GUI! RODE A GUI!

public class Server{ //Servidor Thread pois será rodado em paralelo com o cliente e a GUI.
	  ...
	  try{
		  ...
		  while (true) {
				  ...
				  FileOutputStream target_file = new FileOutputStream(arq);
					try{
						byte [] array_bytes  = new byte[1024]; //cria um array de 1024 bytes, pois será enviado KB por KB.
					      ...
					      int bytes_was_read = 0; //Número de bytes q foram lidos.
					      while(bytes_was_read > -1) //Quando termina de ler o arquivo, o método retorna -1.
					      {
			      ...
			      sock.close(); //fecha soquete após receber arquivo.
		      }		  
		  }catch(Exception e)
		  {
			  e.printStackTrace();
